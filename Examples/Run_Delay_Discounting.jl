...
log_like = compute_log_like(loglike, design_grid, parm_grid, data_grid);
prior = compute_prior(model, parm_grid)
log_post = log.(prior[:])
marg_log_like = compute_marg_log_like(log_post, log_like)
